...
class InvoiceSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data, method):
        ...
        if method == 'POST':
            ...
            for transaction_data in transactions_data:
                transaction_data['line_total'] = transaction_data.get(
                    'price') * transaction_data.get('quantity')
                transaction.append(
                    Transaction.objects.create(invoice=invoice, **transaction_data))
            return invoice
        elif method == 'PUT':
            transaction = []
            validated_data['total_quantity'] = 0
            ...
            transactions_in_db = Transaction.objects.filter(
                invoice_id=validated_data.get('id'))
            transactions_in_db = [
                transaction_id.id for transaction_id in transactions_in_db if transaction_id.id]
            transaction_in_request = [transaction_id.get(
                'id') for transaction_id in transactions_data if transaction_id.get('id')]
            remove_transactions = list(
                set(transactions_in_db) - set(transaction_in_request))

            for remove_transaction in remove_transactions:
                transaction_to_remove = Transaction.objects.filter(
                    id=remove_transaction)
            ...
            for transaction_data in transactions_data:
                ...
                if transaction_data.get('id'):
                    ...
                    update_transaction.product = transaction_data.get(
                        'product')
                    update_transaction.quantity = transaction_data.get(
                        'quantity')
                    update_transaction.price = transaction_data.get('price')
                    update_transaction.line_total = transaction_data.get(
                        'line_total')
...
class InvoiceGetSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Invoice
        fields = (
            'id', 'customer', 'total_quantity', 'total_amount', 'transactions')
